...
select 이름 
from 영화 
where 번호 in(select 영화번호
				from 출연
                where 역 <> 1
                and 배우번호 in (select 번호
									from 배우 
                                    where 이름 = '주진모'));
select b.이름, c.이름 
from 출연 as a
...
select b.개봉연도, count(b.개봉연도) as 작품수
from 출연 as a
left join 영화 as b
on  a.영화번호 = b.번호
left join 배우 as c
